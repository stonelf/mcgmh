...
$moreTypes=array(105=>"卡昂学堂",132=>"般若讲坛",114=>"佛教观察",1=>"弘法足迹",2=>"传承上师",3=>"护生乐园",4=>"视听专区",5=>"显密文库",6=>"佛子心语");
    ...
    foreach ($moreTypes as $typeId=>$typeTitle){
    	$newartlistHTML.='<a href="'.$hostName.'/list/'.$typeId.'.html"><h2>'.$typeTitle.'</h2></a>';
    }
    ...
    while ($row=$dsql->GetObject())
    {
        $newartlistSlide .= "<li><a href='{$hostName}/article/{$row->id}.html'><img style='max-width:320px;max-height:240px;margin:0 auto;' src='{$row->litpic}'><div class='am-slider-desc'>{$row->title}</div></a></li>";
